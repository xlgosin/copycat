...
function render(s){
  ...
  $('running').textContent=s.running?'自动跟单中':'已暂停';
...
async function action(path,body={}){try{await api(path,body);await refresh()}catch(e){$('message').textContent=e.message}}
