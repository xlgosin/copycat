...
function render(s){
  ...
  const warning=s.error || (s.source_stale?'源数据已过期，请先启动网页采集；当前金额仅为历史快照。':'') || (account.hedge_mode?'当前为双向持仓模式，开始跟单前需改为单向。':'');
  ...
  $('orderList').innerHTML=shown.length?shown.map(r=>{
    ...
    const compare=!system&&(r.operation==='OPEN'||r.operation==='CLOSE')?`<div class="compare"><div><small>熬鹰</small>${metric('时间',r.source_time?date(r.source_time):'—')}${metric('价格',r.source_price!=null&&r.source_price!==''?number(r.source_price,8):'—')}${metric('数量',r.source_quantity!=null&&r.source_quantity!==''?number(r.source_quantity,8):'—')}${r.source_close_percent!=null?metric('平仓占比',number(r.source_close_percent,2)+'%'):''}</div><div><small>我</small>${metric('时间',date(r.time))}${metric('价格',r.price!=null&&r.price!==''?number(r.price,8):'—')}${metric('数量',r.quantity!=null&&r.quantity!==''?number(r.quantity,8):'—')}${r.local_close_percent!=null?metric('平仓占比',number(r.local_close_percent,2)+'%'):''}</div></div>${delay?`<small>${delay}</small>`:''}`:'';
...
async function action(path,body={}){
  ...
  else {actionBusy=true;$('start').disabled=true}
