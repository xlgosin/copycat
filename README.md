...
`WATCH_PORTFOLIOS` 是观察源列表，格式 `交易员ID:名称,交易员ID:名称`。只采集开平仓成交并发送钉钉，不读余额、不校验跟单基线、不进入 CopyCat 跟单。以后要加交易员，在列表末尾追加 `,ID:名称` 并重启采集器即可。观察源失败不影响跟单源。
...
- 本金默认100 USDT；比例放大由 `COPY_MULTIPLIER` 决定（最多3）；普通加密资产永续使用逐仓，杠杆由 `FUTURES_LEVERAGE` 决定（最多5）；TradFi 永续使用全仓并固定为5倍杠杆。开仓名义敞口上限由 `MAX_GROSS_NOTIONAL` 决定，不得超过本金×普通合约杠杆（100U×5倍=500U）。这些是开仓前限制，价格上涨后的名义金额可以超过该上限。
- 开仓数量 = 源成交数量 × 当前账户权益（不超过配置的本金上限） / 源带单保证金余额 × 跟单倍率。这里使用源接口字段 `margin_balance`，并非包括跟单资金的 `aum`。计算后还会按当前可用余额、0.5% 名义金额的预留费用和剩余敞口向下缩量，不因为超出少量保证金而整单跳过。
...
- 实盘只支持 USDT 永续、单向持仓、单资产保证金，并将跟出的合约设置为逐仓5倍。同币种已有反向仓位时跳过新开仓。
...
- 开仓信号延迟超过 `MAX_SIGNAL_AGE_SECONDS` 时只记录跳过，绝不补开；该配置默认且硬性封顶为300秒（5分钟）。平仓信号即使延迟仍按源平仓比例处理，避免已有跟单仓位漏平。
...
- 实盘启动前检查 U 本位合约的 BNB 手续费抵扣。首次经页面确认后用 5 USDT 兑换 BNB；此后仅当 BNB 市值低于 3 USDT 时再次提示兑换 5 USDT。兑换意图按币安 quote ID 持久化，状态不确定时不会重复兑换。
...
采集器发现新的开仓/平仓成交后发送成交通知；`PORTFOLIO_ID` 标记为跟单，`WATCH_PORTFOLIOS` 中的交易员按各自名称标记为观察。源成交距发送不超过5分钟时逐笔及时通知，超过5分钟的迟到或重试积压成交按交易员合并成一条摘要，避免历史成交逐条刷屏。首次启用和重启不会补发历史成交，并按源成交 ID 去重。跟单服务只发送真正导致系统暂停的异常通知，不再发送本地成交、跳过/拒绝、待确认或自动恢复消息。用户明确屏蔽的暂停原因不会发送。消息包含方向、操作、源成交时间、数量和价格，时间统一显示为 UTC+8。

采集器成交队列与跟单暂停队列分别持久化到 SQLite，并独立退避重试；发送失败不改变采集或交易状态。通知服务超时后重试仍可能重复到达，可用消息中的通知编号识别。

页面“运行控制”显示跟单服务的钉钉状态；采集器成交队列记录在源数据库中。不自动发送测试成交消息；要停通知可配置 `DINGTALK_ENABLED=false`。
