...
通知覆盖开仓成交、平仓成交、跳过/拒绝订单、异常暂停和待确认订单。后台轮询触发安全暂停时会立即把原因加入通知队列，并与跟单执行线程独立重试。每条包含 CopyCat、熬鹰跟单、运行模式、合约方向、原因；多单使用绿色标识，空单使用红色标识，时间统一显示为 UTC+8。已确认成交附数量、价格、客户订单号，平仓附本次毛盈亏。模拟通知明确标记“模拟”，待确认通知不会把请求数量写成成交数量。历史基线不补发通知。
