...
采集器发现新的熬鹰开仓/平仓成交后发送成交通知；源成交距发送不超过10分钟时逐笔及时通知，超过10分钟的迟到或重试积压成交合并成一条摘要通知，避免历史成交逐条刷屏。首次启用和重启不会补发历史成交，并按源成交 ID 去重。跟单服务只发送真正导致系统暂停的异常通知，不再发送本地成交、跳过/拒绝、待确认或自动恢复消息。用户明确屏蔽的暂停原因不会发送。消息包含方向、操作、源成交时间、数量和价格，时间统一显示为 UTC+8。
