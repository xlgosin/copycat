...
- 本金默认100 USDT；比例放大由 `COPY_MULTIPLIER` 决定（最多3）；普通加密资产永续的合约杠杆由 `FUTURES_LEVERAGE` 决定（最多5），TradFi 永续开仓固定至少使用20倍；开仓名义敞口上限由 `MAX_GROSS_NOTIONAL` 决定，不得超过本金×普通合约杠杆（100U×5倍=500U）。这些是开仓前限制，价格上涨后的名义金额可以超过该上限。
